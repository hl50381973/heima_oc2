一、选择题
1． 若有说明：int a=2, *p=&a, *q=p;，则以下非法的赋值语句是（b）。//d   不可以直接将一个整形术赋值给一个指针变量
A． p=q;           B． *p=*q;         C． a=*q;         D． q=a;

2． 若定义：int a=511, *b=&a；，则printf("%d\n", *b);的输出结果为：d
A． 无确定值       B． a的地址       C． 512           D． 511

3． 已有定义　int a=2, *p1=&a, *p2=&a; 下面不能正确执行的赋值语句是（a）。//b
...
7． 若有语句int *p, a=10; p=&a; 下面均代表地址的一组选项是（b）。// &于＊是一对反操作，也就是＊&或&＊相当于什么都没写
A． a, p, *&a                             B． &*a, &a, *p
C． *&p, *p, &a                           D． &a, &*p, p

8． 若需要建立如图所示的存储结构，且已有说明double *p, x=0.2345; 则正确的赋值语句是（d）。//没图
...
10．有如下语句：int m=6, n=9, *p, *q; p=&m; q=&n; 如图1所示，若要实现下图所示的存储结构，可选用的赋值语句是（）。//没图
...
14．设char *s="\ta\017bc"；则指针变量s指向的字符串所占的字节数是（b）。//?c
A． 9                   B． 5            C．  6             D． 7
...
19．设有如下的程序段：char s[]="girl", *t;   t=s;则下列叙述正确的是（c）。//d
A． s和t完全相同
...
26．若有以下定义和语句：
    ...
    ps=s;
    则对s数组元素的正确引用形式是（d）。
    A． ps+1        B． *(ps+3)        C． ps[0][2]  //c        D． *(ps+1)+3
    
27．不合法的main函数命令行参数表示形式是：（c）。//?
    A． main( int a, char *c[])                  B． main(int argc, char *argv)
    C． main( int arc, char **arv)               D． main( int argv, char*argc[])
    
28．若有说明语句：char  s[]="it is a example．", *t="it is a example.";则以下不正确的叙述（d）。
...
42．以下程序调用findmax函数返回数组中的最大值。在下面划线处应填入的是（c）。
#include "stdio.h"
    findmax( int *a, int n)
    {
        int *p, *s;
